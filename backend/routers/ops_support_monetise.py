...
# ─────────────────────────────────────────────────────────────────────────────
from backend.services.monetisation import (
    get_retail_media_plan, get_audience_segments,
    get_buying_intent, get_supplier_insights,
)

monetise_router = APIRouter(prefix="/api/v1/monetization", tags=["First Party Data Monetisation"])
...
@monetise_router.get("/retail-media")
def retail_media(campaign_id: str = Query(...)):
    result = get_retail_media_plan(campaign_id)
    if "error" in result:
        raise HTTPException(status_code=404, detail=result["error"])
    return result


@monetise_router.get("/audience-segments")
def audience_segments(n_clusters: int = Query(8, ge=4, le=15)):
    return get_audience_segments(n_clusters)


@monetise_router.post("/buying-intent")
def buying_intent(req: IntentRequest):
    return get_buying_intent(req.customer_id, req.product_id)


@monetise_router.get("/supplier-insights")
def supplier_insights(supplier_id: str = Query(...)):
    result = get_supplier_insights(supplier_id)
    if "error" in result:
        raise HTTPException(status_code=404, detail=result["error"])
    return result


@monetise_router.get("/campaigns")
def list_campaigns(limit: int = Query(50, le=500)):
    from backend.utils.data_loader import get_campaigns
    df = get_campaigns()
    return df[["campaign_id","campaign_name","channel","target_segment","status"]].head(limit).to_dict(orient="records")


@monetise_router.get("/suppliers")
def list_suppliers(limit: int = Query(20, le=200)):
    from backend.utils.data_loader import get_suppliers
    df = get_suppliers()
    return df[["supplier_id","name","category","country","reliability_score"]].head(limit).to_dict(orient="records")
